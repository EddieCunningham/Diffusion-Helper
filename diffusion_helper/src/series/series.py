"""Time series representations and operations for diffusion-based models (PyTorch).

Provides a minimal, torch-compatible `TimeSeries` with batching, masks and plotting hooks.
"""

import torch
from typing import Optional, Tuple, Union, Any, Literal, List
import numpy as np
...
def _make_windowed_batches(obj, window_size: int, T: Optional[int] = None):
  ...
  if T is None:
    try:
      ...
      T = obj.batch_size
  idx = torch.arange(T - window_size + 1).unsqueeze(1) + torch.arange(window_size).unsqueeze(0)
  return obj[idx]

################################################################################################################

class TimeSeries(TensorClass):
  ...
  times: torch.Tensor
  vals: torch.Tensor
  mask: Optional[torch.Tensor] = None  # dtype=bool, True if an observation is present at that time step

  def __init__(self,
               times: Union[torch.Tensor, np.ndarray],
               vals: Union[torch.Tensor, np.ndarray],
               mask: Optional[Union[torch.Tensor, np.ndarray, None]] = None):
  ...
  def get_missing_observation_mask(self) -> torch.Tensor:
  ...
  def __getitem__(self, idx: Union[int, slice, torch.Tensor, np.ndarray]):
    # Supports batching along leading dimension(s). Index into batch axes only.
    t = self.times[idx]
    v = self.vals[idx]
    m = self.mask[idx]
    return TimeSeries(t, v, m)
  ...
  def plot(self,
    ...
    """
    from diffusion_helper.src.series.plot import plot_series
  ...
  def plot_multiple_series(series_list: List['TimeSeries'],
    ...
    Returns:
      tuple: (fig, axes) - The matplotlib figure and axes objects
    """
    from diffusion_helper.src.series.plot import plot_multiple_series
